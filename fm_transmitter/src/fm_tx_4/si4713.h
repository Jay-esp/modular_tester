...
// changes made : 
// functions with BOOL did not return anything -> return true
// functions with VOID returned parameters -> return nothing
// variable name PS conflicts with ESP32 library -> renamed
...
class SI4713
{
  public:
    ...
    void RDS_PS(String PS_RDS, uint8_t number);
...
void SI4713::RDS_PS(String PS_RDS, uint8_t number)
{
  char PSArray[9];
  for (uint8_t i = 0; i < 9; i++) {
    PSArray[i] = 0x20;
  }
  PS_RDS.toCharArray(PSArray, 9);
